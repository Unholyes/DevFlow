-- WIP limits are for active flow columns; done/backlog terminals should not enforce WIP.
-- Also clear stale wip_limit values on done columns (they never took effect).

UPDATE public.workflow_stages
SET wip_limit = NULL
WHERE is_done = true AND wip_limit IS NOT NULL;
