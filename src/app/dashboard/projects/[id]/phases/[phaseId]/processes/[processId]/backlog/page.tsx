...
async function ensureBacklogStageId(supabase: any, orgId: string, phaseId: string) {
  ...
  if ((stages ?? []).length > 0) {
    ...
    // Shift existing stage_order down the line.
    // (Use SQL via update per row to avoid relying on RPC; small stage counts.)
  ...
  // No stages exist at all; create a minimal set.
  const rows = [
...
export default async function ProcessBacklogPage({
  params,
}: {
