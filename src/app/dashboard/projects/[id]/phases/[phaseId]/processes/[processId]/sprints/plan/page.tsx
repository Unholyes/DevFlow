...
async function ensureBacklogStageId(supabase: any, orgId: string, phaseId: string) {
  ...
  if ((stages ?? []).length > 0) {
    // Insert a new backlog stage at order 0 without hijacking "To Do".
  ...
  // No stages exist at all; create a minimal Scrum-friendly default set.
  const rows = [
...
async function ensureSprintStartStageId(supabase: any, orgId: string, phaseId: string) {
  ...
  const start = (stages ?? []).find((s: any) => !s.is_backlog && !s.is_done) ?? (stages ?? []).find((s: any) => !s.is_backlog)
  if (start?.id) return start.id as string

  // If we somehow only have a backlog stage, create a simple "To Do" stage.
...
export default async function ProcessSprintPlanningPage({
  ...
  return (
    <SprintPlanningPageClient
      ...
      initialCapacityPoints={(process as any).sprint_capacity_points ?? undefined}
